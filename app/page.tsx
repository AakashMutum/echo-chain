  ...
  try {
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center p-8 text-center text-red-500">
        ...
        <p className="max-w-2xl border p-4 rounded bg-red-50 dark:bg-red-950/20">
          {(error as Error).message}
        </p>
        <div className="mt-8 p-4 bg-muted rounded text-left text-foreground text-sm font-mono">
          <p className="font-bold mb-2">Debug Info:</p>
          <p>SUPABASE_URL_PRESENT: {process.env.NEXT_PUBLIC_SUPABASE_URL ? "YES" : "NO"}</p>
          <p>SUPABASE_ANON_KEY_PRESENT: {process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ? "YES" : "NO"}</p>
          <p className="mt-2 text-xs text-muted-foreground">
            If these are "NO", please check your Vercel Project Settings {">"} Environment Variables.
            And Ensure you have Redeployed after adding them.
          </p>
        </div>
      </div>
    )
